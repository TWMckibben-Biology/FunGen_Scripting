...
run.Vis <- function(x, y) {
  ...
  b.plot <- barplot(dat$samples$lib.size*1e-6, names = 1:10, ylab = "Library size (millions)", xlab = y) #Make barplot of the library sizes
  b.plot
  #return(bplot)
  ...
  cnttab # I am not getting this table back... summary works, but not the table
  # return(cnttab)
  # MDS Plots
...
run.DESeq <- function(x, y) {
  ...
  resOrdered <- res[order(res$padj),] #results reordered by the adjusted pvalue
  #print(head (resOrdered))
  print(summary(resOrdered))
  
  write.csv(as.data.frame(resOrdered),file=paste0(outdir,"/",y,"_DESeq2.csv")) #write results to a new csv
...
run.EdgeR <- function(x, y) {
  ...
  resSig <- res$table[res$table$FDR<0.05,]
  resOrdered <- resSig[order(resSig$logFC),]
  
  write.csv(as.data.frame(resOrdered),file = paste0(outdir,"/",y,"_edgeR.csv")) #write results to a new csv
  cat("The number of sig DE genes is: ", nrow(resOrdered),"\n\n")
}
```
...
run.LimVoo <- function(x, y) {
  ...
  length(which(res$adj.P.Val < 0.1))
  length(which(res$P.Value < 0.01))

  res$Gene <- rownames(res)
  res <- res[,c("Gene", names(res)[1:6])]
  write.csv(as.data.frame(res),file = paste0(outdir,"/",y,"_LimmaVoom.csv")) #write results to a new csv
  etRes <- decideTests(fitContr)
  summary(etRes)
  plotMD(fitContr, column = 1, status = etRes[,1], main = colnames(fitContr)[1], xlim = c(-0.1,20))
}
```
