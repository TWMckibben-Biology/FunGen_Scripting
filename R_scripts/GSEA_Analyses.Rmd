...
gsea_input <- list(f_name = c(file_path_sans_ext(basename(files))), f_path = c(file.path(indir,files)), content = files %>% map(read.csv) %>% map(as.data.frame)) #gsea_input becomes a list of lists, first list has file names, next file paths, and then the data is in content
...
save_plots <- stringr::str_c(outdir,gsea_input$f_name, ".pdf") # can use pwalk to output data with specific names

for (i in seq_along(gsea_input$content)) {
print(head(gsea_input$content[i]))
}


```

Next my logic here is that I have all of these csv files that I want to take in and output a ranked file list for (** as a note, the columns are not uniform. Need to return to DGE_Analyses.Rmd to output columns with the same names). I can apply a function that does those things. 
